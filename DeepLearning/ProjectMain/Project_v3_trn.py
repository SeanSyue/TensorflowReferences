...
TRAIN_FILE = 'C:/bank/data_set/dense/bank_dense2_up7_train.csv'
model_filepath = 'C:/bank/checkpoint/v3_up7/'
SUMMARY_PATH = 'C:\\bank\\summary\\v3_up7\\'
...
def make_dt_dir(return_dt=True):
    dt = datetime.datetime.now()
    dt_str = f'd{dt.day}_h{dt.hour}_m{dt.minute}_s{dt.second}'

    try:
        os.makedirs(f"{model_filepath}/{dt_str}")
    except OSError as e:
        if e.errno != errno.EEXIST:
            raise

    if return_dt is True:
        return dt_str
...
def main():
    ...
    #  依據開始訓練的時間，創建跟這個時間點相對應的資料夾，並顯示出來，方便分別不同次的訓練結果
    dt_str = make_dt_dir()
    print("dt:", dt_str)
