...
_, value_trn = reader.read(queue_trn)
_, value_tst = reader.read(queue_tst)
dec_trn = tf.decode_csv(value_trn, record_defaults=[[0.0]] * 5)
dec_tst = tf.decode_csv(value_tst, record_defaults=[[0.0]] * 5)
features_trn = dec_trn[:-2]
label_trn = dec_trn[-2:]
features_tst = dec_tst[:-2]
label_tst = dec_tst[-2:]
